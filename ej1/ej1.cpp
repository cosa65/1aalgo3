...
int meAlcanzaCable(int D[],int km, int j, int i, tuple<int,int> tup1, tuple<int,int> tup2, tuple<int,int> *tupActual){
  // int ciudAct=get<1>(*tupActual);
  // int cabAct=get<0>(*tupActual);
...
int ejUno(int km, vector<int> ramales) {
  ...
  while(j>=0){
    ciudAct=get<1>(*tupActual);
    cabAct=get<0>(*tupActual);    
    int meAlcanza = meAlcanzaCable(D,km,j,i,tup1,tup2,tupActual);	//Devuelve un int que me indica si ALCANZA, ALCANZA_SI_DESCARTO o NO_ALCANZA
    ...
    }else if(meAlcanza==ALCANZA_SI_DESCARTO && !(acaboDeEmpezar(tupActual))){
    	//if(acaboDeEmpezar(tupActual)) get<1>(*tupActual)++;
